...
### 変更点
- Enhance: 評定方法が「最高評点」のテストで満点に達しておらず再受験できる場合、「再受験可能」と表示するように
- Fix: テストの受験履歴がカード型レイアウトで表示されるMoodle（4.2以降）にて、受験済みのテストが未受験として判定される問題を修正
- Fix: 課題カードのHTMLに文字列 `false` が属性やスタイルとして出力される問題を修正
- Fix: 開始時刻と提出期限が同時刻の場合に、開始時刻が重複して表示される問題を修正
